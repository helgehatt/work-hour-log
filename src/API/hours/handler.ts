...
export default (event: ReturnType<UnionType<typeof actions>>) => {
  switch (event.type) {
    case constants.HOURS_CREATE: return requests.create(event.payload)
    ...
    case constants.HOURS_READ: return requests.read(event.payload)
    ...
    case constants.HOURS_UPDATE: return requests.update(event.payload)
    ...
    case constants.HOURS_DELETE: return requests.delete(event.payload)
